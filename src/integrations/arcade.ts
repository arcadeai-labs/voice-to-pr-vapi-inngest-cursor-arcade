// Arcade — the authenticated side effect (post to Slack) runs through Arcade so
// it is scoped to a REAL user's OAuth grant, not a shared bot token. That
// per-user authorization is the whole point.
//
...
export async function executeTool(
  ...
  if (auth.status !== "completed") {
    // Headless server flow: we can't open a browser, so surface the URL. Once
    // authorized, retries sail through because authorize() is a no-op for this user.
